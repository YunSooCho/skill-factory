"""
Klaviyo Marketing API Actions implementation.
"""
import requests
import time
...
class KlaviyoActions:
    ...
    def _make_request(
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                raise KlaviyoAuthenticationError(
            ...
            raise KlaviyoError(f"Request timeout after {self.timeout} seconds")
        except requests.RequestException as e:
            raise KlaviyoError(f"Request failed: {str(e)}")
    ...
    def get_lists(self, **kwargs) -> Dict[str, Any]:
        """
        get_lists - GET request.
        
        Returns:
            API response as dict
        """
        return self._make_request("GET", "/get_lists", data=kwargs if "GET" in ["POST", "PUT", "PATCH"] else None, params=kwargs if "GET" in ["GET", "DELETE"] else None)
