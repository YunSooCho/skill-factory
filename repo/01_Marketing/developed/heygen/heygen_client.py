...
class HeyGenClient:
    def __init__(self, api_key: str, base_url: str = "https://api.heygen.com/v1", timeout: int = 30, max_retries: int = 3):
    ...
    def _make_request(self, method: str, endpoint: str, json_data: Optional[Dict] = None) -> Dict:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if not response.ok:
                    raise HeyGenAPIError(f"API error {response.status_code}: {response.text}")
                return response.json()
            except requests.RequestException as e:
                if attempt == self.max_retries - 1:
                    raise HeyGenAPIError(f"Request failed: {str(e)}")
                time.sleep(1)
        raise HeyGenAPIError("Max retries exceeded")

    def list_avatars(self, page: int = 1, page_size: int = 20) -> Dict:
        return self._make_request('GET', '/avatars', json_data={'page': page, 'page_size': page_size})

    def get_avatar(self, avatar_id: str) -> Dict:
        return self._make_request('GET', f'/avatars/{avatar_id}')
