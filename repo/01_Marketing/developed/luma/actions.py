"""
Luma Events API Actions implementation.
"""
import requests
import time
...
class LumaActions:
    ...
    def _make_request(
        ...
        try:
            ...
            raise LumaError(f"Request timeout after {self.timeout} seconds")
        except requests.RequestException as e:
            raise LumaError(f"Request failed: {str(e)}")
    ...
    def get_events(self, **kwargs) -> Dict[str, Any]:
        """
        get_events - GET request.
        
        Returns:
            API response as dict
        """
        return self._make_request("GET", "/get_events", data=kwargs if "GET" in ["POST", "PUT", "PATCH"] else None, params=kwargs if "GET" in ["GET", "DELETE"] else None)
