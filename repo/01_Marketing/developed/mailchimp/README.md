...
Mailchimp를 위한 Python API 클라이언트입니다.

## 개요

이 클라이언트는 Mailchimp API에 접근하여 각종 CRUD 작업 및 이벤트 처리를 지원합니다.
