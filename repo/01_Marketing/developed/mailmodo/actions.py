"""
Mailmodo Email API Actions implementation.
"""
import requests
import time
...
from .exceptions import (
    ...
    MailmodoValidationError,
)


class MailmodoActions:
    ...
    def _make_request(
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                raise MailmodoAuthenticationError(
                    message=error_data.get("message", "Authentication failed"),
                    status_code=401,
                    response=error_data,
                )
            elif response.status_code == 404:
                raise MailmodoNotFoundError(
            ...
            raise MailmodoError(f"Request timeout after {self.timeout} seconds")
        except requests.RequestException as e:
            raise MailmodoError(f"Request failed: {str(e)}")
    ...
    def create_campaign(self, **kwargs) -> Dict[str, Any]:
        """
        create_campaign - POST request.
        
        Returns:
            API response as dict
        """
        return self._make_request("POST", "/create_campaign", data=kwargs if "POST" in ["POST", "PUT", "PATCH"] else None, params=kwargs if "POST" in ["GET", "DELETE"] else None)
