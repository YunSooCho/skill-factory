"""
MailerSend Email API Actions implementation.
"""
import requests
import time
...
class MailersendActions:
    ...
    def _make_request(
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                raise MailersendAuthenticationError(
                    message=error_data.get("message", "Authentication failed"),
                    status_code=401,
                    response=error_data,
                )
            elif response.status_code == 404:
                raise MailersendNotFoundError(
                ...
                raise MailersendRateLimitError(
            ...
            raise MailersendError(f"Request timeout after {self.timeout} seconds")
    ...
    def get_templates(self, **kwargs) -> Dict[str, Any]:
        """
        get_templates - GET request.
        
        Returns:
            API response as dict
        """
        return self._make_request("GET", "/get_templates", data=kwargs if "GET" in ["POST", "PUT", "PATCH"] else None, params=kwargs if "GET" in ["GET", "DELETE"] else None)
