"""
Mailchimp Email Marketing API Actions implementation.
"""
import requests
import time
...
class MailchimpActions:
    ...
    def _make_request(
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                raise MailchimpAuthenticationError(
            ...
            raise MailchimpError(f"Request timeout after {self.timeout} seconds")
        except requests.RequestException as e:
            raise MailchimpError(f"Request failed: {str(e)}")
    ...
    def get_campaigns(self, **kwargs) -> Dict[str, Any]:
        """
        get_campaigns - GET request.
        
        Returns:
            API response as dict
        """
        return self._make_request("GET", "/get_campaigns", data=kwargs if "GET" in ["POST", "PUT", "PATCH"] else None, params=kwargs if "GET" in ["GET", "DELETE"] else None)

    def send_campaign(self, **kwargs) -> Dict[str, Any]:
        """
        send_campaign - POST request.
        
        Returns:
            API response as dict
        """
        return self._make_request("POST", "/send_campaign", data=kwargs if "POST" in ["POST", "PUT", "PATCH"] else None, params=kwargs if "POST" in ["GET", "DELETE"] else None)
