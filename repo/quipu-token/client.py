"""
Quipu Token API Client - Spanish Accounting Platform
"""
...
class QuipuTokenError(Exception):
    """Base exception"""

class QuipuTokenRateLimitError(QuipuTokenError):
    """Rate limit"""

class QuipuTokenAuthenticationError(QuipuTokenError):
    """Auth failed"""

class QuipuTokenClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise QuipuTokenRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise QuipuTokenAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise QuipuTokenError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise QuipuTokenError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
