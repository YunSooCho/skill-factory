"""
Misoca API Client - Japanese Invoice Platform
"""
...
class MisocaError(Exception):
    """Base exception"""

class MisocaRateLimitError(MisocaError):
    """Rate limit"""

class MisocaAuthenticationError(MisocaError):
    """Auth failed"""
...
class MisocaClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise MisocaRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise MisocaAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise MisocaError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise MisocaError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
