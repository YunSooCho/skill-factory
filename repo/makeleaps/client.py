"""
MakeLeaps API Client - Japanese Tax & Billing Platform
"""
...
class MakeleapsError(Exception):
    """Base exception"""

class MakeleapsRateLimitError(MakeleapsError):
    """Rate limit"""

class MakeleapsAuthenticationError(MakeleapsError):
    """Auth failed"""

class MakeleapsClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise MakeleapsRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise MakeleapsAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise MakeleapsError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise MakeleapsError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
