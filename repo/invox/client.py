"""
Invox API Client - Invoice & Accounting Platform
"""
...
class InvoxError(Exception):
    """Base exception"""

class InvoxRateLimitError(InvoxError):
    """Rate limit"""

class InvoxAuthenticationError(InvoxError):
    """Auth failed"""
...
class InvoxClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise InvoxRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise InvoxAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise InvoxError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise InvoxError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
