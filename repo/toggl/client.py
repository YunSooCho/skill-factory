"""
Toggl API Client - Time tracking and productivity
"""
...
class TogglError(Exception):
    """Base exception for Toggl errors"""
    pass
...
class TogglAuthenticationError(TogglError):
...
class TogglClient:
    ...
    def __init__(self, access_token: str, timeout: int = 30):
        ...
        Args:
            access_token: Toggl OAuth access token
    ...
    def _handle_response(self, response: requests.Response) -> Dict[str, Any]:
        ...
        if response.status_code == 429:
            raise TogglRateLimitError("Rate limit exceeded")

        if response.status_code == 401:
            raise TogglAuthenticationError("Authentication failed")
        ...
        if response.status_code >= 400:
            ...
            raise TogglError(f"API error ({response.status_code}): {error_msg}")
    ...
    def get_time_entries(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            raise TogglError(f"Request failed: {str(e)}")
