...
class DocusignClient:
    """
    Complete client for docusign integration.
    ...
    def __init__(
        ...
        Args:
            api_key: API key (from env: DOCUSIGN_API_KEY)
            base_url: Base URL (from env: DOCUSIGN_BASE_URL)
            timeout: Request timeout in seconds
            verify_ssl: Whether to verify SSL certificates
        """
        self.api_key = api_key or os.getenv("DOCUSIGN_API_KEY")
        self.base_url = base_url or os.getenv(
            "DOCUSIGN_BASE_URL",
            f"https://api.docusign/v1"
        )
        ...
        if not self.api_key:
            raise ValueError(
                f"API key is required. Set DOCUSIGN_API_KEY environment variable."
            )
