"""
PCA Cloud Kyuyo Hyper API Client - Japanese Payroll System
"""
...
class PcaCloudKyuyoHyperError(Exception):
    """Base exception for PCA Cloud Kyuyo Hyper"""


class PcaCloudKyuyoHyperClient:
    BASE_URL = "https://api-cloud.pc-cloud.jp/hyper/v1"

    def __init__(self, api_key: str, company_code: str, timeout: int = 30):
        """
        Initialize PCA Cloud Kyuyo Hyper client

        Args:
            api_key: PCA Cloud Kyuyo Hyper API key
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        ...
        if resp.status_code >= 400:
            try:
                ...
                raise PcaCloudKyuyoHyperError(f"Error ({resp.status_code}): {error_data}")
            except Exception:
                raise PcaCloudKyuyoHyperError(f"Error ({resp.status_code}): {resp.text}")
    ...
    # Salary & Compensation
    def get_compensation(self, employee_id: str, year: int, month: int) -> Dict[str, Any]:
        """Get compensation details"""
        self._enforce_rate_limit()
        params = {'year': year, 'month': month}
        resp = self.session.get(
            f"{self.BASE_URL}/employees/{employee_id}/compensation",
            params=params,
            timeout=self.timeout
        )
        return self._handle_response(resp)

    def update_compensation(self, employee_id: str, comp_data: Dict) -> Dict[str, Any]:
        """Update compensation"""
        self._enforce_rate_limit()
    ...
    # Reports
    def get_payroll_report(self, year: int, month: int, report_type: str) -> Dict[str, Any]:
        """Get payroll report"""
        self._enforce_rate_limit()
        params = {'year': year, 'month': month, 'report_type': report_type}
