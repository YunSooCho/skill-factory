"""
Cloudgate Uno API Client - Cloud Identity & Access Management
"""
...
class CloudgateUnoError(Exception):
    """Base exception for Cloudgate Uno"""


class CloudgateUnoClient:
    BASE_URL = "https://api.cloudgateuno.com/v1"

    def __init__(self, api_key: str, tenant_id: str, timeout: int = 30):
        """
        Initialize Cloudgate Uno client

        Args:
            api_key: Cloudgate Uno API key
            tenant_id: Tenant ID
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        ...
        if resp.status_code >= 400:
            try:
                ...
                raise CloudgateUnoError(f"Error ({resp.status_code}): {error_data}")
            except Exception:
                raise CloudgateUnoError(f"Error ({resp.status_code}): {resp.text}")
    ...
    def get_access_policies(self) -> Dict[str, Any]:
        """Get access policies"""
        self._enforce_rate_limit()
        resp = self.session.get(f"{self.BASE_URL}/policies", timeout=self.timeout)
        return self._handle_response(resp)
