...
class RaindropClient(object):
    """Async Raindrop.io API client"""
        ...
        try:
            async with self.session.request(method, url, json=data, params=params) as response:
                ...
                if response.status not in (200, 201, 204):
                    ...
                    raise ValueError(f"Raindrop.io API error ({response.status}): {msg}")
                return result
...
async def main():
    ...
    async with RaindropClient(api_key=api_key) as client:
        # TODO: Add usage
        pass
