...
class QuickChartClient(object):
    """Async QuickChart API client"""
        ...
        try:
            async with self.session.request(method, url, json=data, params=params) as response:
                ...
                if response.status not in (200, 201, 204):
                    ...
                    raise ValueError(f"QuickChart API error ({response.status}): {msg}")
                return result
...
async def main():
    ...
    async with QuickChartClient(api_key=api_key) as client:
        # Example usage
        try:
