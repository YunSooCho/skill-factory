...
class HappyScribeClient:
    ...
    def __init__(self, api_key: str):
        """
        Initialize HappyScribeClient API client.
        ...
        try:
            async with self.session.request(
                ...
                if response.status >= 400:
                    ...
                    raise Exception(f"HappyScribeClient API error ({response.status}): {error_msg}")
            ...
            raise Exception(f"Error during HappyScribeClient API request: {str(e)}")


    # ==================== API Methods ====================

    async def transcribe:
        """Placeholder for transcribe.
