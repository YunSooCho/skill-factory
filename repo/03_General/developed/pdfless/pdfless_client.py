...
class PDFlessClient(object):
    """Async PDFless API client"""
    BASE_URL = "https://api.pdfless.app/v1"
        ...
        try:
            async with self.session.request(method, url, json=data, params=params) as response:
                ...
                if response.status not in (200, 201, 204):
                    ...
                    raise ValueError(f"PDFless API error ({response.status}): {msg}")
                return result
...
async def main():
    ...
    async with PDFlessClient(api_key=api_key) as client:
        # Example usage
        try:
