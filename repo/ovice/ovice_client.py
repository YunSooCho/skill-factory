...
class OviceClient(object):
    """Async Ovice API client"""
    BASE_URL = "https://api.ovice.io/v1"
        ...
        try:
            async with self.session.request(method, url, json=data, params=params) as response:
                ...
                if response.status not in (200, 201, 204):
                    ...
                    raise ValueError(f"Ovice API error ({response.status}): {msg}")
                return result
...
async def main():
    ...
    async with OviceClient(api_key=api_key) as client:
        # Example usage
        try:
