"""
Lexoffice API Client - German Accounting Platform
"""
...
class LexofficeAuthenticationError(LexofficeError):
    """Auth failed"""
...
class LexofficeClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise LexofficeRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise LexofficeAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise LexofficeError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise LexofficeError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
