...
class PandadocClient:
    """
    Complete client for pandadoc integration.
    ...
    def __init__(
        ...
        Args:
            api_key: API key (from env: PANDADOC_API_KEY)
            base_url: Base URL (from env: PANDADOC_BASE_URL)
        ...
        if not self.api_key:
            raise ValueError(
                f"API key is required. Set PANDADOC_API_KEY environment variable."
            )
