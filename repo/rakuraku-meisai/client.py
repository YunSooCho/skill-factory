"""
Rakuraku Meisai API Client - Japanese Statement Management
"""
...
class RakurakuMeisaiError(Exception):
    """Base exception"""

class RakurakuMeisaiRateLimitError(RakurakuMeisaiError):
    """Rate limit"""

class RakurakuMeisaiAuthenticationError(RakurakuMeisaiError):
    """Auth failed"""

class RakurakuMeisaiClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise RakurakuMeisaiRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise RakurakuMeisaiAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise RakurakuMeisaiError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise RakurakuMeisaiError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
