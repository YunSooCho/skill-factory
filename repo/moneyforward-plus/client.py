"""
Moneyforward Plus API Client - Extended Moneyforward API
"""
...
class MoneyforwardPlusError(Exception):
    """Base exception"""

class MoneyforwardPlusRateLimitError(MoneyforwardPlusError):
    """Rate limit"""

class MoneyforwardPlusAuthenticationError(MoneyforwardPlusError):
    """Auth failed"""

class MoneyforwardPlusClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise MoneyforwardPlusRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise MoneyforwardPlusAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise MoneyforwardPlusError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise MoneyforwardPlusError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
