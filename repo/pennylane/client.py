"""
Pennylane API Client - French Accounting Platform
"""
...
class PennylaneAuthenticationError(PennylaneError):
    """Auth failed"""
...
class PennylaneClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise PennylaneRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise PennylaneAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise PennylaneError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise PennylaneError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
