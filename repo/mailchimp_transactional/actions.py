"""
Mailchimp Transactional API Actions implementation.
"""
import requests
import time
...
from .exceptions import (
    ...
    MailchimpTransactionalValidationError,
)


class MailchimpTransactionalActions:
    ...
    def _make_request(
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                raise MailchimpTransactionalAuthenticationError(
                    message=error_data.get("message", "Authentication failed"),
                    status_code=401,
                    response=error_data,
                )
            elif response.status_code == 404:
                raise MailchimpTransactionalNotFoundError(
                ...
                raise MailchimpTransactionalRateLimitError(
                    message="Rate limit exceeded", status_code=429
                )
            elif response.status_code >= 400:
                error_data = response.json() if response.text else {}
                raise MailchimpTransactionalError(
            ...
            raise MailchimpTransactionalError(f"Request timeout after {self.timeout} seconds")
        except requests.RequestException as e:
            raise MailchimpTransactionalError(f"Request failed: {str(e)}")
    ...
    def get_stats(self, **kwargs) -> Dict[str, Any]:
        """
        get_stats - GET request.
        
        Returns:
            API response as dict
        """
        return self._make_request("GET", "/get_stats", data=kwargs if "GET" in ["POST", "PUT", "PATCH"] else None, params=kwargs if "GET" in ["GET", "DELETE"] else None)
