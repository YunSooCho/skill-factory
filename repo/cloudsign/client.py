...
class CloudsignClient:
    """
    Complete client for cloudsign integration.
    ...
    def __init__(
        ...
        Args:
            api_key: API key (from env: CLOUDSIGN_API_KEY)
            base_url: Base URL (from env: CLOUDSIGN_BASE_URL)
            timeout: Request timeout in seconds
            verify_ssl: Whether to verify SSL certificates
        """
        self.api_key = api_key or os.getenv("CLOUDSIGN_API_KEY")
        self.base_url = base_url or os.getenv(
            "CLOUDSIGN_BASE_URL",
            f"https://api.cloudsign/v1"
        )
        ...
        if not self.api_key:
            raise ValueError(
                f"API key is required. Set CLOUDSIGN_API_KEY environment variable."
            )
