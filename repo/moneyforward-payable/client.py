"""
Moneyforward Payable API Client - Payables Management System
"""
...
class MoneyforwardPayableError(Exception):
    """Base exception"""

class MoneyforwardPayableRateLimitError(MoneyforwardPayableError):
    """Rate limit"""

class MoneyforwardPayableAuthenticationError(MoneyforwardPayableError):
    """Auth failed"""

class MoneyforwardPayableClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise MoneyforwardPayableRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise MoneyforwardPayableAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise MoneyforwardPayableError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise MoneyforwardPayableError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
