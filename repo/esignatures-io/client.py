...
class EsignaturesIoClient:
    """
    Complete client for esignatures-io integration.
    ...
    def __init__(
        ...
        Args:
            api_key: API key (from env: ESIGNATURES_IO_API_KEY)
            base_url: Base URL (from env: ESIGNATURES_IO_BASE_URL)
            timeout: Request timeout in seconds
            verify_ssl: Whether to verify SSL certificates
        """
        self.api_key = api_key or os.getenv("ESIGNATURES_IO_API_KEY")
        self.base_url = base_url or os.getenv(
            "ESIGNATURES_IO_BASE_URL",
            f"https://api.esignatures.io/v1"
        )
        ...
        if not self.api_key:
            raise ValueError(
                f"API key is required. Set ESIGNATURES_IO_API_KEY environment variable."
            )
