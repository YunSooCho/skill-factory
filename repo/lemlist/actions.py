"""
Lemlist Cold Email API Actions implementation.
"""
import requests
import time
...
from .exceptions import (
    ...
    LemlistValidationError,
)


class LemlistActions:
    ...
    def _make_request(
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                raise LemlistAuthenticationError(
                    message=error_data.get("message", "Authentication failed"),
                    status_code=401,
                    response=error_data,
                )
            elif response.status_code == 404:
                raise LemlistNotFoundError(
            ...
            raise LemlistError(f"Request timeout after {self.timeout} seconds")
        except requests.RequestException as e:
            raise LemlistError(f"Request failed: {str(e)}")
