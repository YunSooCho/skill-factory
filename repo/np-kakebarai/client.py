"""
NP Kakebarai API Client - Payment Management System
"""
...
class NpKakebaraiError(Exception):
    """Base exception"""

class NpKakebaraiRateLimitError(NpKakebaraiError):
    """Rate limit"""

class NpKakebaraiAuthenticationError(NpKakebaraiError):
    """Auth failed"""

class NpKakebaraiClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise NpKakebaraiRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise NpKakebaraiAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise NpKakebaraiError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise NpKakebaraiError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
