"""
Moneyforward Kessai API Client - Settlement Management System
"""
...
class MoneyforwardKessaiError(Exception):
    """Base exception"""

class MoneyforwardKessaiRateLimitError(MoneyforwardKessaiError):
    """Rate limit"""

class MoneyforwardKessaiAuthenticationError(MoneyforwardKessaiError):
    """Auth failed"""

class MoneyforwardKessaiClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise MoneyforwardKessaiRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise MoneyforwardKessaiAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise MoneyforwardKessaiError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise MoneyforwardKessaiError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
