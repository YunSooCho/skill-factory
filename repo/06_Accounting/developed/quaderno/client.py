"""
Quaderno API Client - Tax Compliance Platform
"""
...
class QuadernoRateLimitError(QuadernoError):
    """Rate limit"""

class QuadernoAuthenticationError(QuadernoError):
    """Auth failed"""
...
class QuadernoClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise QuadernoRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise QuadernoAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise QuadernoError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise QuadernoError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
