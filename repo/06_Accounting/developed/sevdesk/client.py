"""
Sevdesk API Client - German Accounting Platform
"""
...
class SevdeskRateLimitError(SevdeskError):
    """Rate limit"""

class SevdeskAuthenticationError(SevdeskError):
    """Auth failed"""
...
class SevdeskClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise SevdeskRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise SevdeskAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise SevdeskError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise SevdeskError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
