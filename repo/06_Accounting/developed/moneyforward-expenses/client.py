"""
Moneyforward Expenses API Client - Expense Management System
"""
...
class MoneyforwardExpensesError(Exception):
    """Base exception"""

class MoneyforwardExpensesRateLimitError(MoneyforwardExpensesError):
    """Rate limit"""

class MoneyforwardExpensesAuthenticationError(MoneyforwardExpensesError):
    """Auth failed"""

class MoneyforwardExpensesClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise MoneyforwardExpensesRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise MoneyforwardExpensesAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise MoneyforwardExpensesError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise MoneyforwardExpensesError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
