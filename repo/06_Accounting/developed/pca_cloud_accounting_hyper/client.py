"""
PCA Cloud Accounting Hyper API Client - Advanced Cloud Accounting
"""
...
class PcaCloudAccountingHyperError(Exception):
    """Base exception"""

class PcaCloudAccountingHyperRateLimitError(PcaCloudAccountingHyperError):
    """Rate limit"""

class PcaCloudAccountingHyperAuthenticationError(PcaCloudAccountingHyperError):
    """Auth failed"""

class PcaCloudAccountingHyperClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise PcaCloudAccountingHyperRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise PcaCloudAccountingHyperAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise PcaCloudAccountingHyperError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise PcaCloudAccountingHyperError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
