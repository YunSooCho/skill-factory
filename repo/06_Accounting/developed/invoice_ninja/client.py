"""
Invoice Ninja API Client - Invoicing & Billing Platform
"""
...
class InvoiceNinjaRateLimitError(InvoiceNinjaError):
    """Rate limit"""

class InvoiceNinjaAuthenticationError(InvoiceNinjaError):
    """Auth failed"""
...
class InvoiceNinjaClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise InvoiceNinjaRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise InvoiceNinjaAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise InvoiceNinjaError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise InvoiceNinjaError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
