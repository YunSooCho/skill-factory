"""
Moneyforward Invoice Three API Client - Invoice Management API v3
"""
...
class MoneyforwardInvoiceThreeError(Exception):
    """Base exception"""

class MoneyforwardInvoiceThreeRateLimitError(MoneyforwardInvoiceThreeError):
    """Rate limit"""

class MoneyforwardInvoiceThreeAuthenticationError(MoneyforwardInvoiceThreeError):
    """Auth failed"""

class MoneyforwardInvoiceThreeClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise MoneyforwardInvoiceThreeRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise MoneyforwardInvoiceThreeAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise MoneyforwardInvoiceThreeError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise MoneyforwardInvoiceThreeError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
