"""
Kanjo Bugyo API Client - Japanese Accounting System
"""
...
class KanjoBugyoError(Exception):
    """Base exception"""

class KanjoBugyoRateLimitError(KanjoBugyoError):
    """Rate limit"""

class KanjoBugyoAuthenticationError(KanjoBugyoError):
    """Auth failed"""

class KanjoBugyoClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise KanjoBugyoRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise KanjoBugyoAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise KanjoBugyoError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise KanjoBugyoError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
