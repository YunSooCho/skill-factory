"""
Invox Issue API Client - Invoice Management System
"""
...
class InvoxIssueError(Exception):
    """Base exception"""

class InvoxIssueRateLimitError(InvoxIssueError):
    """Rate limit"""

class InvoxIssueAuthenticationError(InvoxIssueError):
    """Auth failed"""

class InvoxIssueClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise InvoxIssueRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise InvoxIssueAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise InvoxIssueError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise InvoxIssueError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
