"""
LINE Messaging API Actions implementation.
"""
import requests
import time
...
class LineActions:
    ...
    def _make_request(
        ...
        try:
            ...
            raise LineError(f"Request timeout after {self.timeout} seconds")
        except requests.RequestException as e:
            raise LineError(f"Request failed: {str(e)}")


    def send_message(self, **kwargs) -> Dict[str, Any]:
        """
        send_message - POST request.
        
        Returns:
            API response as dict
        """
        return self._make_request("POST", "/send_message", data=kwargs if "POST" in ["POST", "PUT", "PATCH"] else None, params=kwargs if "POST" in ["GET", "DELETE"] else None)

    def get_profile(self, **kwargs) -> Dict[str, Any]:
        """
        get_profile - GET request.
        
        Returns:
            API response as dict
        """
        return self._make_request("GET", "/get_profile", data=kwargs if "GET" in ["POST", "PUT", "PATCH"] else None, params=kwargs if "GET" in ["GET", "DELETE"] else None)
