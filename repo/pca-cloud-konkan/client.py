...
Complete client for PCA Cloud Konkan warehouse integration.
...
class PcaCloudKonkanAPIClient:
    """
    Complete client for PCA Cloud Konkan warehouse management.
    Supports inventory and shipping operations.
    """
    ...
    def __init__(
        ...
        self.api_key = api_key or os.getenv("PCA_CLOUD_KONKAN_API_KEY")
        self.base_url = base_url or os.getenv("PCA_CLOUD_KONKAN_BASE_URL", "https://api.pcacloud.jp/konkan/v1")
        self.timeout = timeout
        self.verify_ssl = verify_ssl

        if not self.api_key:
            raise ValueError("API key is required. Set PCA_CLOUD_KONKAN_API_KEY environment variable.")
