...
class PushbulletClient(object):
    ...
    async def _request(self, method: str, endpoint: str, data=None, params=None):
        ...
        if params and "apikey_header" == "query":
        ...
        try:
            async with self.session.request(method, url, json=data, params=params) as response:
                ...
                if response.status not in (200, 201, 204):
                    ...
                    raise ValueError(f"Pushbullet API error ({response.status}): {msg}")
                return result
...
async def main():
    ...
    async with PushbulletClient(api_key=api_key) as client:
        # TODO: Add usage
        pass
