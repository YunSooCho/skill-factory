"""
Klippa DocHorizon API Client - Document Processing Platform
"""
...
class KlippaDochorizonError(Exception):
    """Base exception"""

class KlippaDochorizonRateLimitError(KlippaDochorizonError):
    """Rate limit"""

class KlippaDochorizonAuthenticationError(KlippaDochorizonError):
    """Auth failed"""

class KlippaDochorizonClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise KlippaDochorizonRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise KlippaDochorizonAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise KlippaDochorizonError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise KlippaDochorizonError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
