"""
PCA Cloud Accounting API Client - Cloud Accounting Platform
"""
...
class PcaCloudAccountingError(Exception):
    """Base exception"""

class PcaCloudAccountingRateLimitError(PcaCloudAccountingError):
    """Rate limit"""

class PcaCloudAccountingAuthenticationError(PcaCloudAccountingError):
    """Auth failed"""

class PcaCloudAccountingClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise PcaCloudAccountingRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise PcaCloudAccountingAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise PcaCloudAccountingError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise PcaCloudAccountingError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
