...
class NinjaSignClient:
    """
    Complete client for ninja-sign integration.
    ...
    def __init__(
        ...
        Args:
            api_key: API key (from env: NINJA_SIGN_API_KEY)
            base_url: Base URL (from env: NINJA_SIGN_BASE_URL)
            timeout: Request timeout in seconds
            verify_ssl: Whether to verify SSL certificates
        """
        self.api_key = api_key or os.getenv("NINJA_SIGN_API_KEY")
        self.base_url = base_url or os.getenv(
            "NINJA_SIGN_BASE_URL",
            f"https://api.ninja.sign/v1"
        )
        ...
        if not self.api_key:
            raise ValueError(
                f"API key is required. Set NINJA_SIGN_API_KEY environment variable."
            )
