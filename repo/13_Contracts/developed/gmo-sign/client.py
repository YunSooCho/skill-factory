...
class GmoSignClient:
    """
    Complete client for gmo-sign integration.
    ...
    def __init__(
        ...
        Args:
            api_key: API key (from env: GMO_SIGN_API_KEY)
            base_url: Base URL (from env: GMO_SIGN_BASE_URL)
            timeout: Request timeout in seconds
            verify_ssl: Whether to verify SSL certificates
        """
        self.api_key = api_key or os.getenv("GMO_SIGN_API_KEY")
        self.base_url = base_url or os.getenv(
            "GMO_SIGN_BASE_URL",
            f"https://api.gmo.sign/v1"
        )
        ...
        if not self.api_key:
            raise ValueError(
                f"API key is required. Set GMO_SIGN_API_KEY environment variable."
            )
