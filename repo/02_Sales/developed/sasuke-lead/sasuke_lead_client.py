...
class SasukeLeadClient(object):
    """Async Sasuke Lead API client"""
    BASE_URL = "https://api.sasuke-lead.some-api-server"
        ...
        try:
            async with self.session.request(method, url, json=data, params=params) as response:
                ...
                if response.status not in (200, 201, 204):
                    ...
                    raise ValueError(f"Sasuke Lead API error ({response.status}): {msg}")
                return result
...
async def main():
    ...
    async with SasukeLeadClient(api_key=api_key) as client:
        # Example usage
        try:
