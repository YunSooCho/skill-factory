"""
Moneyforward API Client - Japanese Financial Platform
"""
...
class MoneyforwardRateLimitError(MoneyforwardError):
    """Rate limit"""

class MoneyforwardAuthenticationError(MoneyforwardError):
    """Auth failed"""
...
class MoneyforwardClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise MoneyforwardRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise MoneyforwardAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise MoneyforwardError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise MoneyforwardError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
