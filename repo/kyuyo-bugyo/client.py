"""
Kyuyo-Bugyo API Client - Japanese Payroll System
"""
...
class KyuyoBugyoError(Exception):
    """Base exception for Kyuyo-Bugyo"""


class KyuyoBugyoClient:
    BASE_URL = "https://api.kingtime.jp/v1"

    def __init__(self, api_key: str, company_code: str, timeout: int = 30):
        """
        Initialize Kyuyo-Bugyo client

        Args:
            api_key: Kyuyo-Bugyo API key
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        ...
        if resp.status_code >= 400:
            try:
                ...
                raise KyuyoBugyoError(f"Error ({resp.status_code}): {error_data}")
            except Exception:
                raise KyuyoBugyoError(f"Error ({resp.status_code}): {resp.text}")
    ...
    def get_payroll(self, year: int, month: int) -> Dict[str, Any]:
        """Get payroll for specified month"""
    ...
    def calculate_payroll(self, year: int, month: int) -> Dict[str, Any]:
        """Calculate payroll for month"""
        self._enforce_rate_limit()
        params = {'year': year, 'month': month}
        resp = self.session.post(f"{self.BASE_URL}/payroll/calculate", params=params, timeout=self.timeout)
        return self._handle_response(resp)
    ...
    # Bonuses
    def get_bonuses(self, year: Optional[int] = None, season: Optional[int] = None) -> Dict[str, Any]:
        """Get bonus records"""
        self._enforce_rate_limit()
        params = {}
        if year:
    ...
    def add_deduction(self, employee_id: str, deduction_data: Dict) -> Dict[str, Any]:
        """Add deduction"""
        self._enforce_rate_limit()
    ...
    def add_allowance(self, employee_id: str, allowance_data: Dict) -> Dict[str, Any]:
        """Add allowance"""
        self._enforce_rate_limit()
        resp = self.session.post(
            f"{self.BASE_URL}/employees/{employee_id}/allowances",
    ...
    def get_tax_info(self, employee_id: str, year: int, month: int) -> Dict[str, Any]:
        """Get tax information"""
        self._enforce_rate_limit()
        params = {'year': year, 'month': month}
        resp = self.session.get(
