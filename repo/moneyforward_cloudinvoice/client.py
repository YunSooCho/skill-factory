"""
Moneyforward Cloudinvoice API Client - Cloud Invoice Management
"""
...
class MoneyforwardCloudinvoiceError(Exception):
    """Base exception"""

class MoneyforwardCloudinvoiceRateLimitError(MoneyforwardCloudinvoiceError):
    """Rate limit"""

class MoneyforwardCloudinvoiceAuthenticationError(MoneyforwardCloudinvoiceError):
    """Auth failed"""

class MoneyforwardCloudinvoiceClient:
    ...
    def _handle_response(self, resp: requests.Response) -> Dict[str, Any]:
        if resp.status_code == 429:
            raise MoneyforwardCloudinvoiceRateLimitError("Rate limit exceeded")
        if resp.status_code == 401:
            raise MoneyforwardCloudinvoiceAuthenticationError("Authentication failed")
        if resp.status_code >= 400:
            try:
                error_data = resp.json()
                raise MoneyforwardCloudinvoiceError(f"Error ({resp.status_code}): {error_data}")
            except:
                raise MoneyforwardCloudinvoiceError(f"Error ({resp.status_code}): {resp.text}")
        return resp.json()
